...
class listener implements EventSubscriberInterface
{
	...
	public function modify_mcp_modules_display_option($event)
	{
		...
		$is_livesearch_mcp = isset($this->config['live_search_on_off_mcp']) & (bool) $this->config['live_search_on_off_mcp'];
		...
		$this->template->assign_vars(array(
			'LIVE_SEARCH_MIN_NUM_SYMBLOLS_USER_MCP'	=>  isset($this->config['live_search_min_num_symblols_mcp_user']) ? (int) $this->config['live_search_min_num_symblols_mcp_user'] : 0,
			'LIVE_SEARCH_MIN_NUM_SYMBLOLS_FORUM_MCP'	=>  isset($this->config['live_search_min_num_symblols_mcp_forum']) ? (int) $this->config['live_search_min_num_symblols_mcp_forum'] : 0,
			'LIVE_SEARCH_MIN_NUM_SYMBLOLS_GROUP_MCP'	=>  isset($this->config['live_search_min_num_symblols_mcp_group']) ? (int) $this->config['live_search_min_num_symblols_mcp_group'] : 0,
			'LIVE_SEARCH_MAX_ITEMS_TO_SHOW_MCP'			=>  isset($this->config['live_search_max_items_to_show_mcp']) ? (int) $this->config['live_search_max_items_to_show_mcp'] : 0,
		));
	}
	...
	{
		$this->user->add_lang_ext('alg/liveSearch', 'live_search');
		$is_livesearch_acp = isset($this->config['live_search_on_off_acp']) & (bool) $this->config['live_search_on_off_acp'];
		...
		$tab = $this->request->variable('i', '',true);
	...
	public function posting_modify_template_vars($event)
	{
		if ($this->config['live_search_on_off_similartopic'])
		{
			...
			$this->template->assign_vars(array(
				...
				'MIN_CHARS_FORUM'			=> isset($this->config['live_search_min_num_symblols_forum']) ? (int) $this->config['live_search_min_num_symblols_forum'] : 1,
				'MIN_CHARS_USER'			=> isset($this->config['live_search_min_num_symblols_user']) ? (int) $this->config['live_search_min_num_symblols_user'] : 1,
				'MAX_ITEMS_TO_SHOW_FORUM'	=> isset($this->config['live_search_max_items_to_show_forum']) ? (int) $this->config['live_search_max_items_to_show_forum'] : 20,
				'MAX_ITEMS_TO_SHOW_USER'	=> isset($this->config['live_search_max_items_to_show_user']) ? (int) $this->config['live_search_max_items_to_show_user'] : 20,
			));
		}
	}
	...
	public function page_header_after($event)
	{
		...
		$this->template->assign_vars(array(
			...
			'MIN_CHARS_FORUM'			=> isset($this->config['live_search_min_num_symblols_forum']) ? (int) $this->config['live_search_min_num_symblols_forum'] : 1,
			'MIN_CHARS_TOPIC'			=> isset($this->config['live_search_min_num_symblols_topic']) ? (int) $this->config['live_search_min_num_symblols_topic'] : 1,
			'MIN_CHARS_USER'			=> isset($this->config['live_search_min_num_symblols_user']) ? (int) $this->config['live_search_min_num_symblols_user'] : 1,
			'MAX_ITEMS_TO_SHOW_FORUM'	=> isset($this->config['live_search_max_items_to_show_forum']) ? (int) $this->config['live_search_max_items_to_show_forum'] : 20,
			'MAX_ITEMS_TO_SHOW_TOPIC'	=> isset($this->config['live_search_max_items_to_show_topic']) ? (int) $this->config['live_search_max_items_to_show_topic'] : 20,
			'MAX_ITEMS_TO_SHOW_USER'	=> isset($this->config['live_search_max_items_to_show_user']) ? (int) $this->config['live_search_max_items_to_show_user'] : 20,
